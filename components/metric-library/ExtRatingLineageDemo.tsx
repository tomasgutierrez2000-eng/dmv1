...
import ExtRatingDemoNarrationPanel from './ext-rating-demo/ExtRatingDemoNarrationPanel';
import { EXT_RATING_DEMO_STEPS, resolveSelector } from './ext-rating-demo/extRatingDemoSteps';

/* ────────────────────────────────────────────────────────────────────────────
 * ExtRatingLineageDemo — main orchestrator for the External Rating guided demo
 *
 * Simplified (no variant picker). Portal-rendered to document.body.
 * Composes overlay, narration panel, and control bar. Communicates
 * side-effects (expand rollup level) back to the parent via onSideEffect.
 * ──────────────────────────────────────────────────────────────────────────── */
...
interface ExtRatingLineageDemoProps {
  onClose: () => void;
  onSideEffect: (fx: Partial<ExtRatingDemoSideEffects>) => void;
}

/* ── useExtRatingDemoEngine — simplified demo engine (no variant) ────────── */

function useExtRatingDemoEngine({ onClose, onSideEffect }: ExtRatingLineageDemoProps) {
  ...
  const totalSteps = EXT_RATING_DEMO_STEPS.length;

  /* ── measure target rect ───────────────────────────────────────────────── */

  const measureTarget = useCallback((stepIndex: number) => {
    const step = EXT_RATING_DEMO_STEPS[stepIndex];
    if (!step) {
      setTargetRect(null);
      return;
    }
  ...
  const scrollAndMeasure = useCallback(
    (stepIndex: number) => {
      ...
      if (step.onEnter) {
        const fx: Partial<ExtRatingDemoSideEffects> = {};
...
export default function ExtRatingLineageDemo({ onClose, onSideEffect }: ExtRatingLineageDemoProps) {
  const engine = useExtRatingDemoEngine({ onClose, onSideEffect });
