...
const TABLES: Record<string, TableDef> = {
  ...
  fps: {
    ...
    fields: [
      { name: 'facility_id', sampleValue: '12345' },
      { name: 'all_in_rate_pct', sampleValue: '6.2500' },
      { name: 'base_rate_pct', sampleValue: '5.0000' },
      { name: 'as_of_date', sampleValue: '2024-12-31' },
    ],
  },
  fm: {
...
const DIMENSION_DEMOS: DimensionDemo[] = [
  {
    ...
    label: 'Facility',
    icon: Table2,
    description: 'Direct interest income for a single loan',
    ...
    steps: [
      {
        ...
        narration:
          'We start at the exposure snapshot table. This is a daily photo of every loan the bank has. For facility 12345, the "drawn amount" tells us the borrower has taken out $120 million of their credit line.',
      },
      {
        kind: 'join',
        highlightTable: 'fps',
        ...
        fieldsToShow: ['facility_id', 'all_in_rate_pct', 'as_of_date'],
        narration:
          'Next, we follow the facility_id link to the pricing snapshot. This table records the all-in interest rate — base rate plus spread. For facility 12345, the all-in rate is 6.25% annually.',
      },
      {
        kind: 'join',
        highlightTable: 'fm',
        ...
        narration:
          'We also look up the loan\'s master record in facility_master. This gives us the loan\'s identity: it\'s a "CRE Multifamily" loan. This table also holds the foreign keys we\'ll need for higher-level rollups.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Now we have everything we need. The Interest Income formula multiplies the drawn amount by the rate: $120M × 6.25% / 100 = $7,500,000. This is the annualized interest income from this single facility.',
        sampleResult: '$120M × 6.25% = $7,500,000',
      },
    ],
  },
  {
    ...
    label: 'Counterparty',
    icon: Users,
    description: 'SUM of interest income across a borrower\'s loans',
    ...
    steps: [
      {
        ...
        narration:
          'We start the same way — at the exposure snapshot. But this time we pull drawn amounts for ALL facilities belonging to one borrower. Apex Properties has two CRE loans.',
      },
      {
        kind: 'join',
        highlightTable: 'fps',
        ...
        arrowTo: 'fps',
        joinKey: 'facility_id + as_of_date',
        fieldsToShow: ['facility_id', 'all_in_rate_pct'],
        narration:
          'For each facility, we look up its all-in rate from the pricing snapshot. Different loans may have different rates — a multifamily loan at 6.25% and an office loan at 5.75%.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'First, we calculate interest income for each facility individually. Facility A: $120M × 6.25% = $7,500,000. Facility B: $85M × 5.75% = $4,887,500. Each facility now has its own dollar amount.',
        sampleResult: '$7.5M and $4.9M per facility',
      },
      {
        kind: 'join',
        highlightTable: 'fm',
      ...
      },
      {
        ...
        narration:
          'We follow the counterparty_id to the counterparty table to resolve the borrower\'s identity: "Apex Properties." All facilities sharing this counterparty_id belong to the same borrower.',
      },
      {
        kind: 'aggregate',
        ...
        fieldsToShow: [],
        narration:
          'Because Interest Income is a dollar amount (not a ratio), we simply SUM the per-facility values: $7,500,000 + $4,887,500 = $12,387,500. No weighting needed — dollars are additive.',
        sampleResult: 'SUM = $12,387,500',
      },
    ],
  },
  {
    ...
    label: 'Desk',
    icon: Briefcase,
    description: 'SUM of interest income for a trading desk',
    ...
    steps: [
      ...
      },
      {
        ...
        fieldsToShow: ['facility_id', 'all_in_rate_pct'],
        narration:
          'Look up each facility\'s all-in rate from the pricing snapshot. Same join as before — facility_id + as_of_date links the exposure to its pricing.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Calculate per-facility interest income first. Every rollup level above facility depends on having these individual dollar amounts computed.',
        sampleResult: 'Per facility incomes computed',
      },
      {
        kind: 'join',
        highlightTable: 'fm',
      ...
      },
      {
        ...
        fieldsToShow: [],
        narration:
          'Group all facilities by desk, then SUM their interest income: $7,500,000 + $4,887,500 = $12,387,500 for the CRE Lending Desk. Simple addition — no weighting or averaging needed for currency metrics.',
        sampleResult: 'Desk SUM = $12,387,500',
      },
    ],
  },
  {
    ...
    label: 'Portfolio',
    icon: FolderTree,
    description: 'SUM of interest income for a portfolio (group of desks)',
    tables: ['fes', 'fps', 'fm', 'ebt', 'calc', 'sumagg'],
    steps: [
      {
        kind: 'source',
        ...
        fieldsToShow: ['facility_id', 'drawn_amount'],
        narration:
          'Starting from exposure data again. For portfolio-level Interest Income, we need to aggregate across all desks that roll up to the same portfolio.',
      },
      {
        kind: 'join',
        highlightTable: 'fps',
        ...
        fieldsToShow: ['facility_id', 'all_in_rate_pct'],
        narration:
          'Pricing lookup — same as every other level. Each facility\'s rate is retrieved to compute its individual interest income.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Compute per-facility interest income first. This is always the prerequisite step.',
        sampleResult: 'Per facility incomes computed',
      },
      {
        kind: 'join',
      ...
      },
      {
        ...
        fieldsToShow: [],
        narration:
          'Group all facilities whose taxonomy leaf traces up to the same L2 portfolio, then SUM their interest income. Result: one total for the entire Commercial Real Estate portfolio.',
        sampleResult: 'Portfolio SUM = $18,132,500',
      },
    ],
  },
  {
    ...
    icon: PieChart,
    description: 'SUM of interest income at the department level',
    tables: ['fes', 'fps', 'fm', 'ebt', 'calc', 'sumagg'],
    steps: [
      {
        kind: 'source',
      ...
      },
      {
        ...
        fieldsToShow: ['facility_id', 'all_in_rate_pct'],
        narration:
          'Pricing values are looked up per facility, as always.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Compute per-facility interest income — the foundation for all aggregation.',
        sampleResult: 'Per facility incomes computed',
      },
      {
        kind: 'join',
      ...
      },
      {
        ...
        fieldsToShow: [],
        narration:
          'Group ALL facilities in the bank whose taxonomy path traces to the same root department. SUM their interest income to get a single board-level revenue number for the entire Lending Division.',
        sampleResult: 'LoB SUM = $18,132,500',
      },
    ],
  },
];
...
function TableCard({
  ...
  return (
    <g style={{
      ...
      ...(isActive ? { animation: 'iittd-fadeIn 0.4s ease-out' } : {}),
    }}>
      {isActive && (
        <>
          <rect x={x - 6} y={y - 6} width={w + 12} height={h + 12} rx={16} fill="none" stroke={glowColor} strokeWidth={1} opacity={0.2} style={{ filter: 'blur(6px)' }} />
          <rect x={x - 3} y={y - 3} width={w + 6} height={h + 6} rx={13} fill="none" stroke={glowColor} strokeWidth={2} opacity={0.5} style={{ animation: 'iittd-glow 2s ease-in-out infinite' }} />
        </>
      )}
      <rect
...
function AnimatedArrow({
  ...
  return (
    <g style={{ opacity: isActive ? 1 : isVisited ? 0.4 : 0, transition: 'opacity 0.5s' }}>
      ...
      <path ref={pathRef} d={pathD} fill="none" stroke={color} strokeWidth={isActive ? 2.5 : 1.5} markerEnd="url(#ii-arrow-active)" style={drawStyle} />
      {isActive && joinKey && (
        <g style={{ animation: 'iittd-fadeIn 0.6s ease-out 0.4s both' }}>
...
export default function IntIncomeTableTraversalDemo() {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
      <style>{`
        @keyframes iittd-glow {
          0%, 100% { opacity: 0.5; }
          50% { opacity: 0.2; }
        }
        @keyframes iittd-fadeIn {
          from { opacity: 0; transform: translateY(4px); }
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes iittd-slideUp {
      ...
      <div className="px-5 py-4 border-b border-white/5 bg-black/20">
        ...
        <div className="h-0.5 bg-gray-800 rounded-full mb-3">
          <div
            className="h-full bg-gradient-to-r from-amber-500 via-blue-500 to-orange-500 transition-all duration-700 ease-out rounded-full"
        ...
        {activeStep === -1 ? (
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-gray-400">
              Press <strong className="text-white">Play</strong> to watch how the <strong className="text-orange-300">{demo.label}</strong> dimension
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={play}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-500/15 border border-orange-500/30 text-orange-300 font-bold text-sm hover:bg-orange-500/25 transition-colors"
              >
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div key={`narration-${selectedDim}-${activeStep}`} className="flex-1 min-w-0 max-w-2xl" style={{ animation: 'iittd-slideUp 0.4s ease-out' }}>
            ...
            <div className="flex flex-col items-center gap-2 flex-shrink-0">
              <div className="flex items-center gap-1.5">
                ...
                {isPlaying ? (
                  ...
                  <button onClick={play} className="w-10 h-10 rounded-xl bg-orange-500/15 border border-orange-500/30 flex items-center justify-center text-orange-300 hover:bg-orange-500/25 transition-colors" title="Play">
      ...
      <div className="px-2 pt-6 pb-3 overflow-x-auto overflow-y-hidden">
        ...
        >
          <defs>
            <marker id="ii-arrow-active" markerWidth={8} markerHeight={8} refX={7} refY={4} orient="auto">
      ...
      <div className="px-5 py-3 border-t border-white/5 bg-white/[0.01]">
        <div className="flex items-center gap-4 text-[9px] text-gray-500">
          ...
          <span className="ml-auto font-medium">Interest Income = Drawn_Amount × All_In_Rate_Pct / 100</span>
        </div>
      </div>
    </div>
  );
