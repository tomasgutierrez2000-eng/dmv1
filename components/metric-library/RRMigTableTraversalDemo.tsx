...
const TABLES: Record<string, TableDef> = {
  cro: {
    ...
    layer: 'L2',
    shortName: 'Rating Observation',
    fields: [
      { name: 'counterparty_id', sampleValue: '7890' },
      { name: 'rating_value', sampleValue: '3' },
      { name: 'prior_rating_value', sampleValue: '2' },
      { name: 'rating_type', sampleValue: 'INTERNAL' },
    ],
  },
  fes: {
    id: 'fes',
    ...
    fields: [
      ...
      { name: 'gross_exposure_usd', sampleValue: '$20,000,000' },
      { name: 'counterparty_id', sampleValue: '7890' },
  ...
  cp: {
    ...
    fields: [
      { name: 'counterparty_id', sampleValue: '7890' },
      { name: 'counterparty_name', sampleValue: 'Acme Industries' },
      { name: 'industry_code', sampleValue: 'Real Estate' },
    ],
  },
  ebt: {
    id: 'ebt',
  ...
  wtdagg: {
    id: 'wtdagg',
    name: 'Exposure-Weighted Avg',
    layer: 'CALC',
    shortName: 'Weighted Average',
    fields: [],
  },
};
...
const DIMENSION_DEMOS: DimensionDemo[] = [
  {
    ...
    label: 'Facility',
    icon: Table2,
    description: 'Exposure-weighted notch change for a single facility',
    tables: ['cro', 'fes', 'fm', 'cp', 'calc'],
    steps: [
      {
        kind: 'source',
        highlightTable: 'cro',
        fieldsToShow: ['counterparty_id', 'rating_value', 'prior_rating_value'],
        narration:
          'We start at the counterparty rating observation table. This stores the bank\'s internal credit ratings for every borrower. For counterparty 7890, the current rating is 3 ("Moderate Risk") and the prior rating was 2 ("Low Risk").',
      },
      {
        kind: 'join',
        highlightTable: 'fes',
        ...
        fieldsToShow: ['facility_id', 'gross_exposure_usd', 'counterparty_id'],
        narration:
          'We follow counterparty_id to the exposure snapshot to find all facilities belonging to this borrower. Facility 12345 has a gross exposure of $20 million \u2014 this is the dollar amount at risk.',
      },
      {
        kind: 'join',
        highlightTable: 'fm',
        ...
        narration:
          'Facility master confirms this is a "CRE Term Loan" and gives us the product type and organizational assignment.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Now we compute: Notch Change = 3 \u2212 2 = +1 (one-notch downgrade). This facility\'s weighted contribution is +1 \u00d7 $20M = +$20M to the numerator.',
        sampleResult: 'Notch = +1, Wtd = +$20M',
      },
    ],
  },
  {
    ...
    label: 'Counterparty',
    icon: Users,
    description: 'Exposure-weighted average notch change across a borrower\'s facilities',
    tables: ['cro', 'fes', 'fm', 'cp', 'calc', 'wtdagg'],
    steps: [
      {
        kind: 'source',
        highlightTable: 'cro',
        fieldsToShow: ['counterparty_id', 'rating_value', 'prior_rating_value'],
        narration:
          'Start at the rating observation table. We pull the current and prior ratings for the borrower. Acme Industries moved from rating 2 to 3 (one of their counterparty\'s facilities) and from 3 to 2 (another).',
      },
      {
        kind: 'join',
        highlightTable: 'fes',
        ...
        fieldsToShow: ['facility_id', 'gross_exposure_usd', 'counterparty_id'],
        narration:
          'For each rating change, we need ALL facilities belonging to this counterparty. Acme has two facilities with exposures of $20M and $15M.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Calculate notch change per facility: Fac A: +1 \u00d7 $20M = +$20M. Fac B: \u22121 \u00d7 $15M = \u2212$15M.',
        sampleResult: '+$20M and \u2212$15M per facility',
      },
      {
        kind: 'join',
        highlightTable: 'cp',
        ...
        narration:
          'Resolve the borrower identity: "Acme Industries." All facilities under this counterparty_id are grouped together.',
      },
      {
        kind: 'aggregate',
        highlightTable: 'wtdagg',
        arrowFrom: 'cp',
        arrowTo: 'wtdagg',
        fieldsToShow: [],
        narration:
          'Exposure-weighted average: (+$20M + (\u2212$15M)) / ($20M + $15M) = +$5M / $35M = +0.14. Slight net deterioration \u2014 the larger CRE loan\'s downgrade outweighs the revolver\'s upgrade.',
        sampleResult: 'Wtd Avg = +0.14',
      },
    ],
  },
  {
    ...
    label: 'Desk',
    icon: Briefcase,
    description: 'Exposure-weighted average notch change for a trading desk',
    tables: ['cro', 'fes', 'fm', 'ebt', 'calc', 'wtdagg'],
    steps: [
      {
        kind: 'source',
        highlightTable: 'cro',
        fieldsToShow: ['counterparty_id', 'rating_value', 'prior_rating_value'],
        narration:
          'Rating observation gives us current and prior ratings for all counterparties. The desk view will group by which trading desk manages each facility.',
      },
      {
        kind: 'join',
        highlightTable: 'fes',
        ...
        fieldsToShow: ['facility_id', 'gross_exposure_usd', 'counterparty_id'],
        narration:
          'Match each rating change to its associated facilities and their exposures.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Calculate notch changes weighted by exposure for each facility.',
        sampleResult: 'Per facility weighted changes',
      },
      {
        kind: 'join',
        highlightTable: 'fm',
        ...
        narration:
          'The facility master has lob_segment_id \u2014 the foreign key that links each loan to the bank\'s organizational tree.',
      },
      {
        kind: 'join',
        highlightTable: 'ebt',
        ...
        narration:
          'The taxonomy table resolves the desk. Segment 301 = "CRE Lending Desk." Every facility with this lob_segment_id belongs to this desk.',
      },
      {
        kind: 'aggregate',
        highlightTable: 'wtdagg',
        ...
        fieldsToShow: [],
        narration:
          'Group by desk and compute exposure-weighted average: CRE Desk: (+1\u00d7$20M + 0\u00d7$25M) / $45M = +0.44 (deterioration). The CRE desk\'s only rating event was a downgrade.',
        sampleResult: 'CRE Desk = +0.44',
      },
    ],
  },
  {
    ...
    label: 'Portfolio',
    icon: FolderTree,
    description: 'Exposure-weighted average notch change for a portfolio (group of desks)',
    tables: ['cro', 'fes', 'fm', 'ebt', 'calc', 'wtdagg'],
    steps: [
      {
        kind: 'source',
        ...
        fieldsToShow: ['counterparty_id', 'rating_value', 'prior_rating_value'],
        narration:
          'Starting from rating observations for all counterparties. Portfolio aggregation spans across all desks under one portfolio.',
      },
      {
        kind: 'join',
        highlightTable: 'fes',
        ...
        fieldsToShow: ['facility_id', 'gross_exposure_usd', 'counterparty_id'],
        narration:
          'Retrieve exposures for all facilities affected by rating changes.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Compute per-facility notch changes weighted by exposure.',
        sampleResult: 'Per facility weighted changes',
      },
      {
        kind: 'join',
        ...
        fieldsToShow: ['facility_id', 'lob_segment_id'],
        narration:
          'Get lob_segment_id from facility master \u2014 the entry point into the org hierarchy.',
      },
      {
        kind: 'join',
        highlightTable: 'ebt',
        ...
        narration:
          'Enter taxonomy at desk level. But we need the PARENT \u2014 the portfolio this desk belongs to.',
      },
      {
        kind: 'join',
        highlightTable: 'ebt',
        ...
        narration:
          'Self-join: desk\'s parent_segment_id (30) gives us the L2 portfolio "Commercial Real Estate." Multiple desks share this parent.',
      },
      {
        kind: 'aggregate',
        highlightTable: 'wtdagg',
        ...
        fieldsToShow: [],
        narration:
          'Exposure-weighted average across ALL facilities in the portfolio: (\u2212$15M) / $100M = \u22120.15. Net improvement across the entire portfolio.',
        sampleResult: 'Portfolio = \u22120.15',
      },
    ],
  },
  {
    ...
    icon: PieChart,
    description: 'Exposure-weighted average notch change at the department level',
    tables: ['cro', 'fes', 'fm', 'ebt', 'calc', 'wtdagg'],
    steps: [
      {
        kind: 'source',
        ...
        fieldsToShow: ['counterparty_id', 'rating_value', 'prior_rating_value'],
        narration:
          'For the highest rollup \u2014 Business Segment \u2014 we start from all rating observations across the bank.',
      },
      {
        kind: 'join',
        highlightTable: 'fes',
        ...
        fieldsToShow: ['facility_id', 'gross_exposure_usd', 'counterparty_id'],
        narration:
          'All facility exposures are retrieved for weighting.',
      },
      {
        kind: 'result',
        highlightTable: 'calc',
        ...
        fieldsToShow: [],
        narration:
          'Per-facility notch changes computed \u2014 the foundation for all aggregation.',
        sampleResult: 'Per facility scores computed',
      },
      {
        kind: 'join',
        highlightTable: 'fm',
        ...
        fieldsToShow: ['facility_id', 'lob_segment_id'],
        narration:
          'Retrieve lob_segment_id \u2014 entry point into organizational hierarchy.',
      },
      {
        kind: 'join',
        highlightTable: 'ebt',
        ...
        fieldsToShow: ['managed_segment_id', 'parent_segment_id', 'lob_l3_name'],
        narration:
          'Enter taxonomy at desk level. Now walk ALL the way up to the ROOT.',
      },
      {
        kind: 'join',
        highlightTable: 'ebt',
        ...
        narration:
          'RECURSIVE self-join: Desk (301) \u2192 Portfolio (30) \u2192 Department (3, root). The root "Lending Division" is the Business Segment.',
      },
      {
        kind: 'aggregate',
        highlightTable: 'wtdagg',
        ...
        fieldsToShow: [],
        narration:
          'Exposure-weighted average across ALL facilities tracing to this root. The board-level credit quality score for the entire lending division.',
        sampleResult: 'LoB Score = \u22120.15',
      },
    ],
  },
];
...
function TableCard({
  ...
  return (
    <g style={{
      ...
      ...(isActive ? { animation: 'rrm-fadeIn 0.4s ease-out' } : {}),
    }}>
      {isActive && (
        <>
          <rect x={x - 6} y={y - 6} width={w + 12} height={h + 12} rx={16} fill="none" stroke={glowColor} strokeWidth={1} opacity={0.2} style={{ filter: 'blur(6px)' }} />
          <rect x={x - 3} y={y - 3} width={w + 6} height={h + 6} rx={13} fill="none" stroke={glowColor} strokeWidth={2} opacity={0.5} style={{ animation: 'rrm-glow 2s ease-in-out infinite' }} />
        </>
      )}
      <rect
...
function AnimatedArrow({
  ...
  return (
    <g style={{ opacity: isActive ? 1 : isVisited ? 0.4 : 0, transition: 'opacity 0.5s' }}>
      ...
      <path ref={pathRef} d={pathD} fill="none" stroke={color} strokeWidth={isActive ? 2.5 : 1.5} markerEnd="url(#rrm-arrow-active)" style={drawStyle} />
      {isActive && joinKey && (
        <g style={{ animation: 'rrm-fadeIn 0.6s ease-out 0.4s both' }}>
...
export default function RRMigTableTraversalDemo() {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
      <style>{`
        ...
        @keyframes rrm-fadeIn {
          from { opacity: 0; transform: translateY(4px); }
          to { opacity: 1; transform: translateY(0); }
        }
        @keyframes rrm-slideUp {
      ...
      <div className="px-5 py-4 border-b border-white/5 bg-black/20">
        ...
        {activeStep === -1 ? (
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div key={`narration-${selectedDim}-${activeStep}`} className="flex-1 min-w-0 max-w-2xl" style={{ animation: 'rrm-slideUp 0.4s ease-out' }}>
      ...
      <div className="px-2 pt-6 pb-3 overflow-x-auto overflow-y-hidden">
        ...
        >
          <defs>
            <marker id="rrm-arrow-active" markerWidth={8} markerHeight={8} refX={7} refY={4} orient="auto">
      ...
      <div className="px-5 py-3 border-t border-white/5 bg-white/[0.01]">
        <div className="flex items-center gap-4 text-[9px] text-gray-500">
          ...
          <span className="ml-auto font-medium">{`Migration Score = \u03A3(Notch_Change \u00d7 Exposure) / \u03A3(Exposure)`}</span>
        </div>
      </div>
    </div>
  );
