...
export default function IntExpenseDemoFormulaAnimation({ formulaKey, variant, stepIndex }: Props) {
  switch (formulaKey) {
    case 'drawn-build':
  ...
  return (
    <div className="rounded-lg bg-white/[0.03] border border-gray-800 p-3 space-y-2">
      ...
      >
        ...
        <span className="text-gray-500 font-mono">$120,000,000</span>
      </div>
      <div
      ...
      >
        <span className="text-rose-400">Drawn (Funded) Balance</span>
        <span className="text-white font-mono">$95,000,000</span>
      </div>
      <div
        className="text-[9px] text-gray-600 mt-1 transition-all duration-500"
        style={{ opacity: revealed > 1 ? 1 : 0 }}
      >
        79% utilization — borrower has drawn $95M of their $120M credit line
      </div>
    </div>
  );
}
    ...
    <div className="rounded-lg bg-white/[0.03] border border-gray-800 p-3 space-y-1.5">
      ...
      >
        <div className="flex items-center gap-1.5">
          <span className="text-rose-400 font-mono font-bold flex-shrink-0">+</span>
          <span className="text-gray-300">Matched-Maturity Funding Rate</span>
        </div>
        <span className="text-gray-400 font-mono">3.80%</span>
      </div>
      <div
        className="flex items-center justify-between text-xs transition-all duration-500"
        style={{ opacity: revealed > 1 ? 1 : 0, transform: revealed > 1 ? 'translateX(0)' : 'translateX(16px)' }}
      >
        <div className="flex items-center gap-1.5">
          <span className="text-rose-400 font-mono font-bold flex-shrink-0">+</span>
          <span className="text-gray-300">Liquidity Premium</span>
        </div>
        <span className="text-gray-400 font-mono">0.70%</span>
      </div>
      <div
      ...
      >
        <span className="text-rose-400">FTP Cost of Funds</span>
...
function MultiplyResult({ stepIndex }: { stepIndex: number }) {
  ...
  return (
    <div className="rounded-lg bg-white/[0.03] border border-gray-800 p-4 space-y-2 text-center">
      ...
      >
        <span className="text-[10px] text-gray-500">Interest Expense =</span>
        <span className="text-2xl font-black text-rose-400 ml-2 tabular-nums">$4,275,000</span>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <div className="space-y-1 text-xs font-mono">
          <div className="flex justify-between text-gray-400">
            <span>Facility A</span><span className="text-rose-400">$4,275,000</span>
          </div>
          <div className="flex justify-between text-gray-400">
            <span>Facility B</span><span className="text-rose-400">$1,660,000</span>
          </div>
        </div>
        <div
          ...
          style={{ opacity: revealed > 1 ? 1 : 0 }}
        >
          Total = <span className="text-rose-400 font-bold">$5,935,000</span>
      ...
      >
        ...
        <div className="text-[10px] text-gray-400 leading-relaxed">
          ...
          Interest Expense is a <span className="text-rose-300 font-semibold">dollar amount</span> — pure addition
          is always correct.
        </div>
      </div>
    </div>
      ...
      {FACILITIES.map((f, i) => (
        ...
        >
          ...
          <div className="flex items-center gap-2 font-mono flex-shrink-0 ml-2">
            <span className="text-rose-400 text-[10px]">{fmtM(f.drawnAmount)}</span>
            <span className="text-gray-600">&times;</span>
            <span className="text-amber-400 text-[10px]">{fmtRate(f.costOfFundsPct)}</span>
            <span className="text-gray-600">=</span>
            <span className="text-white font-bold">{fmt(f.interestExpense)}</span>
          </div>
        </div>
      ))}
    </div>
          ...
          <div key={cp.name} className="space-y-1">
            ...
            {cpFacilities.map((f, i) => (
              ...
              >
                ...
                <span className="text-rose-400">{fmt(f.interestExpense)}</span>
              </div>
            ))}
            <div
            ...
            >
              ...
              <span className="text-rose-300">{fmt(cp.interestExpense)}</span>
            </div>
          </div>
        );
      })}
      ...
      {DESKS.map((d, i) => (
        ...
        >
          ...
          <div className="text-xs font-mono text-gray-300">
            ...
            <span className="text-rose-400 font-bold">{fmt(d.interestExpense)}</span>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="rounded-lg bg-white/[0.03] border border-gray-800 p-3 space-y-2">
      ...
      {DESKS.map((d, i) => (
        ...
        >
          ...
          <span className="text-rose-400">{fmt(d.interestExpense)}</span>
        </div>
      ))}
      <div
