...
import { EXT_RATING_DEMO_STEPS } from './extRatingDemoSteps';
import ExtRatingDemoFormulaAnimation from './ExtRatingDemoFormulaAnimation';

/* ────────────────────────────────────────────────────────────────────────────
 * ExtRatingDemoNarrationPanel — fixed right-side panel with narration,
...
interface ExtRatingDemoNarrationPanelProps {
  currentStep: number;
  totalSteps: number;
}
...
export default function ExtRatingDemoNarrationPanel({
  currentStep,
  totalSteps,
}: ExtRatingDemoNarrationPanelProps) {
  ...
  const step = EXT_RATING_DEMO_STEPS[currentStep];
  if (!step) return null;
  ...
  return (
    <div className="fixed top-0 right-0 bottom-[72px] w-[340px] z-[52] bg-[#0a0a0a]/95 backdrop-blur-xl border-l border-gray-800 flex flex-col">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4" key={currentStep}>
        <div className="animate-ext-rating-fade-in space-y-4">
          {/* Title */}
          ...
          {step.insight && (
            <div className="rounded-xl border border-pwc-orange/30 bg-pwc-orange/5 px-4 py-3 flex items-start gap-3">
              ...
              <div className="text-xs text-gray-300 leading-relaxed">{step.insight}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <style jsx>{`
        .animate-ext-rating-fade-in {
          animation: extRatingDemoFadeIn 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        @keyframes extRatingDemoFadeIn {
          from {
            opacity: 0;
