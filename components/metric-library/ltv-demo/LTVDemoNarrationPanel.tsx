...
import LTVDemoFormulaAnimation from './LTVDemoFormulaAnimation';

/* ────────────────────────────────────────────────────────────────────────────
 * LTVDemoNarrationPanel — fixed right-side panel (340px)
 *
 * Adapted from the DSCR DemoNarrationPanel but simplified:
 * no VariantKey — LTV has a single formula, all step fields are strings.
 * ──────────────────────────────────────────────────────────────────────────── */

interface LTVDemoNarrationPanelProps {
  currentStep: number;
  totalSteps: number;
}
...
export default function LTVDemoNarrationPanel({
  currentStep,
  totalSteps,
}: LTVDemoNarrationPanelProps) {
  ...
  const step = LTV_DEMO_STEPS[currentStep];
  if (!step) return null;
  ...
  return (
    <div className="fixed top-0 right-0 bottom-[72px] w-[340px] z-[52] bg-[#0a0a0a]/95 backdrop-blur-xl border-l border-gray-800 flex flex-col">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4" key={currentStep}>
        <div className="animate-ltv-fade-in space-y-4">
      ...
      <style jsx>{`
        .animate-ltv-fade-in {
          animation: ltvDemoFadeIn 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        @keyframes ltvDemoFadeIn {
          from {
            opacity: 0;
