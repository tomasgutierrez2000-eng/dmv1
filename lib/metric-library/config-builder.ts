...
import { buildTraversalConfig, buildFallbackTraversalConfig } from './traversal-builder';
...
function inferTraversalConfig(
  item: CatalogueItem,
  _metric?: L3Metric | null,
): TraversalConfig | undefined {
  // Load traversal source data from visualization-configs preset (generated by calc:sync)
  const presets = loadPresets();
  const preset = presets[item.item_id] ?? presets[item.abbreviation] ?? {};
  const traversalData = (preset as Record<string, unknown>).traversal_source_data as TraversalSourceData | undefined;

  if (traversalData) {
    return buildTraversalConfig(item, traversalData);
  }

  // Fallback for metrics without YAML-synced traversal data
  return buildFallbackTraversalConfig(item);
}
