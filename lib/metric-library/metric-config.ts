...
// ═══════════════════════════════════════════════════════════════
// Traversal Source Data (generated by calc:sync from YAML)
// ═══════════════════════════════════════════════════════════════

/** Source table info extracted from YAML for traversal config generation. */
export interface TraversalSourceTable {
  /** YAML alias (e.g. "fm", "fla", "ebt") */
  alias: string;
  /** Schema-qualified name (e.g. "l2.facility_master") */
  qualified_name: string;
  /** Table name only (e.g. "facility_master") */
  table_name: string;
  /** Data layer */
  layer: 'L1' | 'L2' | 'L3';
  /** Join type from YAML */
  join_type: 'BASE' | 'INNER' | 'LEFT';
  /** SQL join condition (null for BASE table) */
  join_on: string | null;
  /** Fields with roles and descriptions */
  fields: Array<{
    name: string;
    role: 'MEASURE' | 'DIMENSION' | 'FILTER' | 'JOIN_KEY';
    description: string;
  }>;
  /** Business-context narration for this table's role in the metric */
  purpose: string;
}

/** Pre-computed traversal source data stored in visualization-configs.json. */
export interface TraversalSourceData {
  /** Ordered source tables from YAML with join info and field roles */
  source_tables: TraversalSourceTable[];
  /** Per-level list of active table aliases (detected from formula_sql) */
  per_level_tables: Partial<Record<RollupLevelKey, string[]>>;
  /** Context for generating narration at runtime */
  narration_context: {
    metric_description: string;
    formula_text: string;
    aggregation_type_by_level: Partial<Record<RollupLevelKey, string>>;
  };
}
