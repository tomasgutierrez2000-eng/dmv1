...
async function main() {
  ...
  if (!chain) {
    ...
    const mockCPs: EnrichedCounterparty[] = Array.from({ length: 10 }, (_, i) => ({
      ...
      basel_risk_grade: i < 5 ? 'IG' : 'HY',
      call_report_counterparty_type: 'C&I_DOMESTIC',
      external_rating_fitch: i < 5 ? 'A' : 'BBB',
      external_rating_moodys: i < 5 ? 'A2' : 'Baa2',
      external_rating_sp: i < 5 ? 'A' : 'BBB',
      fr2590_counterparty_type: 'NON_FINANCIAL',
      internal_risk_rating: i < 5 ? 'A' : 'BBB',
      is_affiliated_flag: 'N',
      is_central_counterparty_flag: 'N',
      is_financial_institution_flag: 'N',
      is_insider_flag: 'N',
      is_multilateral_dev_bank_flag: 'N',
      is_parent_flag: 'N',
      is_public_sector_entity_flag: 'N',
      is_regulated_entity_flag: 'N',
      is_sovereign_flag: 'N',
      lei_code: `529900MOCK${String(i + 1).padStart(8, '0')}`,
      lgd_unsecured: 0.45,
      pd_annual: 0.10 + i * 0.05,
      regulatory_counterparty_type: 'CORPORATE',
      y14_obligor_type: 'LARGE_CORPORATE',
    ...
    const mockFacs: EnrichedFacility[] = mockCPs.map((cp, i) => ({
      ...
      facility_name: `MockFacility-${(i + 1) * 10}`,
      ...
      interest_rate_reference: 'SOFR',
      interest_rate_spread_bps: 100 + i * 15,
      all_in_rate_pct: 5.0 + i * 0.15,
      day_count_convention: 'ACT/360',
      amortization_type: 'BULLET',
      is_revolving_flag: 'Y',
      is_active_flag: 'Y',
      facility_reference: `MOCK-FAC-${(i + 1) * 10}`,
