...
async function loadFactorySQL(client: pg.Client) {
  ...
  try {
    ...
    const cleanupQueries = [
      // L2 event tables (children of L1)
      "DELETE FROM l2.stress_test_breach WHERE stress_test_result_id IN (SELECT stress_test_result_id FROM l2.stress_test_result WHERE counterparty_id >= 1730)",
      "DELETE FROM l2.stress_test_result WHERE counterparty_id >= 1730",
      "DELETE FROM l2.deal_pipeline_fact WHERE counterparty_id >= 1730",
      "DELETE FROM l2.amendment_change_detail WHERE amendment_event_id IN (SELECT amendment_event_id FROM l2.amendment_event WHERE counterparty_id >= 1730)",
