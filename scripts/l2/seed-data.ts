...
import {
  TIER_BASE_SPREADS, TIER_BASE_RW, CURRENCY_BASE_RATES,
  TIER_INT_RATINGS, TIER_EXT_RATINGS, RATING_MIGRATION,
} from '../shared/seed-config';
...
// Tier-level base configs imported from scripts/shared/seed-config.ts
...
export function getL2SeedValue(
  ...
  switch (tableName) {
    ...
    case 'collateral_snapshot': {
      ...
      // NOI current = time-varying income generated by collateral asset (CRE property)
      if (columnName === 'noi_current_amt') return noi(idx);
      break;
    }
