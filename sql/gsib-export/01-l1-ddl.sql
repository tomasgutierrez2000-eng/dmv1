...
-- Drop and recreate for clean load (avoids FK errors from leftover tables).
...
CREATE TABLE IF NOT EXISTS l1.credit_event_type_dim (
  ...
  default_trigger_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (default_trigger_flag IN ('Y','N')),
...
CREATE TABLE IF NOT EXISTS l1.credit_status_dim (
  ...
  default_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (default_flag IN ('Y','N')),
...
CREATE TABLE IF NOT EXISTS l1.amendment_status_dim (
  ...
  is_terminal_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (is_terminal_flag IN ('Y','N')),
  active_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (active_flag IN ('Y','N')),
  created_ts TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_ts TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS l1.default_definition_dim (
  ...
  credit_event_trigger_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (credit_event_trigger_flag IN ('Y','N')),
  materiality_threshold_amt DECIMAL(18,2)
);

CREATE TABLE IF NOT EXISTS l1.internal_risk_rating_bucket_dim (
  internal_risk_rating_bucket_code VARCHAR(50) NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS l1.counterparty_role_dim (
  ...
  is_risk_bearing_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (is_risk_bearing_flag IN ('Y','N')),
  description VARCHAR(2000),
  is_active_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (is_active_flag IN ('Y','N')),
  created_ts TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_ts TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS l1.collateral_eligibility_dim (
  ...
  effective_to_date DATE,
  eligible_flag CHAR(1) NOT NULL DEFAULT 'N' CHECK (eligible_flag IN ('Y','N')),
